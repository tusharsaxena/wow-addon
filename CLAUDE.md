...
Current version: **1.10.0** (in `.claude-plugin/plugin.json`).
...
- **Artifact directories in the target addon.** Three specs write dated or persistent artifacts into the addon they operate on, and each owns its own tree: `review` → `docs/reviews/<date>/`, `standards-audit` → `docs/audits/<date>/`, `pending` → `docs/pending/LEDGER.md`. The dated bundles are **frozen history** — `pending` reads their execution plans to find unexecuted rows but must never edit them. `LEDGER.md` is the opposite: long-lived and merged across runs, so a "defer for now" decision survives and isn't re-interviewed.
...
None tracked. (The `TODO/FIXME` strings that appear in `commands/diff.md`, `commands/sync-docs.md`, and — densely — `commands/pending.md` are those specs *describing their own behavior*, not project TODOs. `pending.md` is the one to watch: a grep for pending work across this repo will always light it up.)
