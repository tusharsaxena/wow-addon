...
Current version: **1.14.0** (in `.claude-plugin/plugin.json`).
...
- `commands/*.md` — 13 slash-command specs (`/wow-addon:<name>`), each also invocable as a Skill of the same name. Two of them (`review.md`, `standards-audit.md`) are thin **wrappers that dispatch to the subagent of the same name**; the other eleven act directly.
...
- **A Ka0s addon's `docs/` is exactly three canonical files** — `ARCHITECTURE.md`, `testing.md`, `smoke-tests.md` (plus the generated `test-cases.md` and topic-detail docs). There is **no** `docs/agent-context.md`; the root `CLAUDE.md` stub is the only agent brief in the repo. Never state the count without naming the members — an unfilled slot is what makes a spec reconstruct the deleted file from memory.
