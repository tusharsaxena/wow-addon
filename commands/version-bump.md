...
- **"Version History" table** at the bottom of the README — this lists OLD versions as facts, not "the current version". Add a NEW row for the new version (with a short blank "release notes" placeholder for the user to fill); do NOT rewrite existing rows.
...
For the README "Version History" table: insert a NEW row at the top (or wherever the table is ordered to put the latest), with the new version and a placeholder release-notes cell like `_TODO: notes_`. Do NOT modify existing rows.

For `CHANGELOG.md`: if an "Unreleased" or `## [Unreleased]` header exists, rewrite it to `## [X.Y.Z] — YYYY-MM-DD` with today's date. Do NOT generate the change list — that's the user's job.
...
- **Don't write a CHANGELOG entry's body from scratch** — only rewrite an existing "Unreleased" header. If no CHANGELOG, don't create one.
