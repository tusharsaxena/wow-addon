---
description: Scaffold a new Ka0s WoW addon that is born compliant with the Ka0s WoW Addon Standard. Fetches the NEW_ADDON.md playbook + the standards context pack from the WowAddonStandards repo at runtime and follows them to the letter — Ace3 skeleton, the modular layout, MIT, the canonical docs/ trio (ARCHITECTURE.md, testing.md, smoke-tests.md), and a CLAUDE.md stub so future agents build the rest against the standard. The context pack is read at runtime and never written into the addon.
...
**CRITICAL — the context pack is never stored in the addon.** You fetch `NEW_ADDON_CONTEXT.md` to a scratch path and build from it. Creating `docs/agent-context.md` — under that name or any other — is a compliance failure (`documentation-§3`, anti-pattern #49). The addon's `docs/` holds exactly `ARCHITECTURE.md`, `testing.md` and `smoke-tests.md`, plus the generated `test-cases.md` and topic-detail docs; the root `CLAUDE.md` stub is the repo's only agent brief.
...
2. **Work from the context pack — never write it into the repo.** `standards/NEW_ADDON_CONTEXT.md` is *scaffolding*: read it from your scratch copy, build from it, and leave it there. Do **NOT** create `docs/agent-context.md` or any other stored copy of it (`documentation-§3`, anti-pattern #49) — every question it answers is answered the moment the addon exists, and a stored copy loads as *working context*, so a stale one gets **followed**. The addon's `docs/` carries exactly three canonical files — `ARCHITECTURE.md`, `testing.md`, `smoke-tests.md` — plus the generated `test-cases.md`; the only agent brief in the repo is the short root `CLAUDE.md` **stub** (see `documentation`), with the durable per-addon context in `docs/ARCHITECTURE.md` and `docs/testing.md`.
