...
2. `$RAW/standards/NEW_ADDON_CONTEXT.md` — the full context pack: kickstart walkthrough, the modular starter tree, the starter snippets, hard-rules cheat sheet, and the Definition-of-Done checklist. It is the **source of detail** for every starter, including the setup files that wire the shared library modules — read the snippets from the pack rather than inventing them, and do not restate them here. **Its contents get dropped into the new addon** (see below).
