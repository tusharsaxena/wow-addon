...
4. **Fill in the starters** — TOC (fixed field order + `#`-section file listing), including the `## X-Standard:` line pointing at the standards repo; entry file, compat shims, locale, database/migrations, schema-driven settings, and the message bus — from the context pack's starter snippets.
   ...
   **The diagnostics dump is born wired, not added later** (`debug-logging-§14`, a MUST in every addon since standard v2.68.0). The scaffold carries `modules/Diagnostics.lua`, the one **sections** file: it writes the addon's own report sections and nothing else, because the line buffer, the per-section `pcall`, the begin and end markers, the cap and the `Add` loop belong to the library helper (`RunDiagnostics`, anti-pattern #90). The debug-console setup file's descriptor names the addon's `brandName` and points at those sections, and its stub answers `DebugVerb` and `RunDiagnostics` like every other member. The slash descriptor carries a `diagnostics` row in `NS.COMMANDS`, and the `debug` handler tests `diagnostics` **first** through `DebugVerb`, so `/<slash> diagnostics` and `/<slash> debug diagnostics` run the same report. Those are the only two forms: no `diag`, `dump` or other alias, as a row, a debug word or a descriptor `aliases` entry. `diagnostics` is a reserved live verb, so it answers while the addon is disabled. The playbook and the context pack carry the skeleton; take it from there.
...
6. **Write the README to `documentation-§1`'s canonical structure**, and **run the de-AI pass over it before the first commit.** `documentation-§1` MUSTs a **de-AI writing pass** on every `README.md` edit (anti-pattern #77): run the `/humanize` skill, or audit against a published AI-writing pattern catalogue, and fix what it finds **before** the change is committed. A brand-new README is the case the rule exists for: every word of it was just generated, with no human draft underneath, so it carries the tells at full strength. It binds this file only — the `CLAUDE.md` stub, `DEPENDENCIES.md` and the `docs/` trio are contributor surfaces and are exempt. What the pass looks for, so it is not a vibe check: uniform paragraph and sentence length (the strongest signal, ahead of vocabulary), `**Bold lead.**` openers on every bullet, see-saw pairs, the same rhetorical move repeated across sections, rhetorical-question openers, rule-of-three padding, em-dash overuse, and the AI vocabulary set (delve, leverage, robust, seamless, testament to, showcase, foster). Removal is half of it — prose with every tell stripped and no voice put back reads as a press release, which is its own tell. Take the section order and every template from the fetched playbook, and note four things it now states which older Ka0s READMEs get wrong, because a scaffold that copies an existing addon's README inherits all four:
   ...
   - **`## Reporting a bug` is a MUST, and its text is fixed** (`documentation-§1` item 9, between `## Troubleshooting` and `## Issues and feature requests`). Copy the body from the fetched section **exactly**, with the addon's real slash in place of `/<slash>` and nothing else changed: the three steps (`debug on` and reproduce, `diagnostics`, then open the window with `debug`, press **Copy** and include the whole output with the bug report) and the closing line about one copy carrying both. It names **no destination** and carries **no** GitHub or issues link, because players send the report to the maintainer privately. The de-AI pass does not reword it. The Troubleshooting table **SHOULD** carry the row pointing at it.
   - **`## Credits` is optional and external-only** (`documentation-§1` item 12) — third-party artwork, a font, a sound pack, another author's work. It is the **last** section, after `## Version History`. A new addon with nothing external to credit ships no `## Credits` section at all.
