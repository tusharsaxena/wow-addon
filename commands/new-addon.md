...
6. **Write the README to `documentation-§1`'s canonical structure.** Take the section order and every template from the fetched playbook, and note three things it now states which older Ka0s READMEs get wrong, because a scaffold that copies an existing addon's README inherits all three:
