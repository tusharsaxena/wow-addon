...
**Two of them are generated and are not synced by hand:** `docs/test-cases.md` (the inventory the harness emits) and the automated-test record — `docs/automated-tests/RESULTS.md` plus the frozen per-run bundles (`automated-tests-§1`, `automated-tests-§4`). Read both — a count claim elsewhere in the docs must agree with `test-cases.md`, and the newest `RESULTS.md` row dates the record — but never edit their numbers. `test-cases.md` is refreshed by regenerating it; the record is regenerated at **release** by `/wow-addon:bump-version`. A hand-edited complexity record is worse than an absent one because it reads as measured (anti-pattern #51). If the newest row is older than the last release, that is a **finding** to report, not something to fix here.
...
For each doc file:
- **README.md**: keep its overall shape. Update each section to reflect current state. Don't invent sections that weren't there. Preserve the user's voice — match the existing tone, formatting, and emoji usage (or absence). Four exceptions, where the shape itself is wrong and `documentation-§1` says so:
  ...
  - **`## Reporting a bug` is a MUST with fixed text** (`documentation-§1` item 9, between `## Troubleshooting` and `## Issues and feature requests`). If it is missing, add it: this is the one section you create rather than preserve, because every addon ships the diagnostics dump (`debug-logging-§14`). If it is present, bring its body back to the standard's text **verbatim apart from the slash**, which is the addon's real one: `debug on` and reproduce, `diagnostics`, then open the window with `debug`, press **Copy** and include the entire output with the bug report, followed by the one closing line about a single copy carrying both. It names **no destination** and carries **no** GitHub or issues link; if a README says "paste it into a GitHub issue" or links the tracker there, that wording comes out. Don't reword the fixed text to match the README's voice. A Troubleshooting row that tells players to paste logs some other way points at this section instead.
