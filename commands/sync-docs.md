...
Before doing anything else, check the addon's doc layout against the Ka0s WoW Addon Standard's fixed structure (documentation). The **root** ships exactly three docs plus `LICENSE`, and never a fourth: a **full** `README.md`, a **stub** `CLAUDE.md`, and `DEPENDENCIES.md` (documentation-§7). Everything else lives under `docs/`, which carries the canonical trio — `docs/ARCHITECTURE.md`, `docs/testing.md` and `docs/smoke-tests.md` — plus the five **required** topic-detail docs: the generated `docs/test-cases.md`, `docs/performance.md`, `docs/perf-runs/README.md`, `docs/automated-tests/README.md` and the generated `docs/automated-tests/RESULTS.md`, and any further topic-detail docs. A standalone `docs/complexity.md` is **retired** (v2.19.0) — finding one is a pre-adoption finding to report, never a doc to keep in sync. There is **no** `docs/agent-context.md` (documentation-§3). This layout is fixed; it does **not** vary with addon size.
