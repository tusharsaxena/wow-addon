...
One `gh` call sequence per decided item. **Space them out** — a few seconds between mutations — and write idempotently: re-read the issue before editing if a call failed, so a timeout doesn't produce a double comment.
