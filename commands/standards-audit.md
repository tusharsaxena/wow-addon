...
**The disabled state is newly auditable, and a clean result there is the thing to question.** `slash-commands-§7` makes disable total — every registration actually unregistered, every timer cancelled, every owned frame hidden at the source, no SavedVariables write reachable from a game event, and a two-verb slash surface — where the old `§2` carried a SHOULD about refusing feature verbs and a twelve-verb live list that is now deleted. Every addon in the collection implements disable as a **draw gate** (the flag as one rung of a show ladder, or an early return per handler), which is `anti-patterns` #85 and which reads as correct from every surface a player can see; the upstream audit that produced the rule found **11 of 11 failing**, with 34 live event registrations, 13 running timers and 18 SavedVariables writes among the survivors. So a summary reporting this area clean, or resting on a prior bundle's verdict, is measuring against a rule that no longer exists — send it back. Two matching non-findings, in the other direction: adoption is **blocked upstream** until a LibKa0s tag ships `LibKa0s-Lifecycle-1.0`, so the gap belongs to the library rather than to the addon's execution plan, and the absence of `/<slash> lock` / `/<slash> unlock` is a declined **MAY** that owes no deviation row at all.
