...
|---|---|
| `/wow-addon:new-addon <Name>` | Scaffold a new addon born compliant with the Ka0s WoW Addon Standard. Fetches the `NEW_ADDON.md` playbook + context pack and follows them: Ace3 skeleton, the modular layout, MIT, the standards brief dropped into the addon's `docs/`, and a `CLAUDE.md` stub. The shared subsystems — chat printer, debug console, options toolkit, slash dispatcher, perf harness, test harness — are **consumed** from the vendored `LibKa0s` library and its test kit, one setup file per adopted module, rather than hand-built. |
...
| `/wow-addon:finalize-multi-repo [repos]` | Finish a changeset that spans several sibling repos. Auto-detects which siblings changed (or takes the list you pass), derives the dependency chain from evidence — a vendored shared library is finalized and **pushed** before the consumers that carry it, and a doc line citing `../<Repo>/…` is an edge like any other — then, per repo: `sync-docs` (content only, never scaffolding, since nobody is there to confirm per repo), that repo's own gate including the vendor-drift diff, `commit`, merge any feature branch with `--no-ff`, push, and delete the branch with `-d`. Independent repos run in parallel, one agent per repo. A red gate, a merge conflict or a rejected push stops that repo alone and is reported with its real output. The one command that deliberately pushes. |
