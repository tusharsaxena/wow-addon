...
2. `$RAW/standards/01_STANDARD.md` — the canonical rules: the `§`-sections and the §19 anti-pattern list. This is *what you check against*.
3. Any further file those two reference and you need to complete the audit (e.g. `$RAW/standards/00_EXECUTIVE_SUMMARY.md`, `$RAW/standards/ADDONS.md` for the addon's prefix). Fetch on demand.
...
**Hard stop.** If you cannot resolve `AUDIT.md` **and** `standards/01_STANDARD.md` (no network, repo moved, 404), do not improvise an audit from memory. Stop and tell the user exactly which fetch failed and what you tried — an audit measured against an unreadable standard is worthless.

Record the standard version you resolved (e.g. the version/date at the top of `01_STANDARD.md`) so the run is reproducible; the playbook wants this noted in `01_CURRENT_STATE.md`.
...
- Measure the addon against every `§` of the standard and the §19 anti-pattern list; record one deviation per MUST/SHOULD it fails or partially meets.
- Catalogue deviations in `02_DEVIATIONS.md`, each with a **stable per-addon-prefixed ID** (2–3 letters from the addon name), the `§` violated, MUST/SHOULD severity, a one-line description, and a fix direction. Reuse a prior audit's prefix and IDs for deviations that recur.
...
If this list and the fetched `AUDIT.md` ever disagree, **the fetched playbook wins**; if the playbook and `01_STANDARD.md` disagree on *what* to check, the standard wins.
