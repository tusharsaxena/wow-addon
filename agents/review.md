...
- **The two surfaces, and the one verb people get wrong.** While disabled the dispatcher answers exactly `enable` and `help`; everything else, including the bare command and an unknown verb, prints one tagged refusal line naming `/<slash> enable`. `disable` is the exception that is **not** a refusal — it is an alias onto a schema write, so writing false over false echoes `<enablePath> = false`; a refusal there answers `/<slash> disable` with a line telling the player to type `enable`, which reads as the addon having misunderstood. The launcher's left-click prints that same line and writes nothing, while right-click still opens the panel (`launcher-§2`). Host-side refusal wording is a finding in its own right: the line comes from the dispatcher, so every addon's refusal is one string.
