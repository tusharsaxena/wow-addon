...
- Each finding gets a stable ID (`F-001`, `F-002`, ...) plus: file:line, one-sentence problem, one-sentence impact, category tag (e.g. `[taint]`, `[design]`, `[ux]`, `[perf]`, `[naming]`, `[locale]`, `[deprecated-api]`, `[tests]`, `[complexity]`, `[lint]`, `[upstream]`).
...
- **Test and complexity movement** — the pass count before and after, whether `docs/test-cases.md` and the README `[tests]` badge moved in the same change, and any watch-list entry in `docs/complexity.md` these changes are expected to move (to be confirmed by the next release's regeneration, not regenerated here).
